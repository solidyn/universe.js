...
SSI.Universe = function(options, container) {
	...
	this.addEarth = function(earthOptions) {
		...
		objectLibrary.addObject("earth", earthMesh);
	...
	// objectName
	// propogator
	// modelLocation <-- TODO: Take out
	this.addSpaceObject = function(spaceObject) {
		var jsonLoader = new THREE.JSONLoader();
		jsonLoader.load({
			model : spaceObject.modelLocation,
			callback : function(geometry) {
				var spacecraftMesh = new THREE.Mesh(geometry, new THREE.MeshFaceMaterial());
				...
				controller.addGraphicsObject({
					...
					update : function() {
						...
						spacecraftMesh.position.set(location.x, location.y, location.z);
						this.draw();
					},
					draw : function() {
						core.draw(this.id, spacecraftMesh);
					}
				});
			}
		});
	...
	// object is a Three.js JSON 3D model
	this.addModelToLibraryFromJSON = function(name, object, returnCallback) {
		...
		jsonLoader.load({
			...
			callback : function(geometry) {
				...
				objectLibrary.addObject(name, geometry);
				returnCallback();
			}
		});
	}
